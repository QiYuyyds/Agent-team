...
This app is planned as a mobile client for the desktop AChat host. It should not run SQLite, LLM SDKs, workspace tools, or Next.js API routes locally. It connects to the desktop host over Tailscale/tailnet or LAN and uses `/api/mobile/*` endpoints for snapshot, events, chat, approvals, and `ask_user` responses.
