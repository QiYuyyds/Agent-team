...
AgentHub 是 local-first 单用户 AI Agent 协作平台，当前使用 SQLite 作为唯一持久化层，上下文拼装通过 `conversation_context.py` 硬编码完成，无向量检索、全文检索、知识图谱或结构化记忆能力。
...
本项目（路径 B）将 AGI-memory 核心能力融合进 AgentHub，SQLite 全量替换为 PostgreSQL，保持单用户设计（`user_id="default_user"`），前端零改动。
...
- 从 AGI-memory 搬运 Prompt 装配器（Slot/Schema/Source/Assembler）并对接 AgentHub 数据源
...
**理由**: AgentHub 后端已全 async（FastAPI + async SQLAlchemy），引入 threading 会破坏一致性

### D5: Prompt 装配器 Source 适配 AgentHub 数据源

**选择**: 6 种 Source 分别对接 AgentHub 现有数据
...
**理由**: 保持 AGI-memory 装配器架构优势，同时复用 AgentHub 已有数据
...
**[R3] AGI-memory 与 AgentHub 的 embedding 模型差异** → AGI-memory 默认 1024 维，AgentHub 需确认 Embedding API 可用性。Mitigation: 配置化 embedding_model/rag_milvus_dim；无 Embedding API 时降级为 TF-IDF 近似。
