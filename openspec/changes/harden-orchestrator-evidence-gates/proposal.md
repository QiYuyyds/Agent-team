...
AChat needs a stronger collaboration harness: planning must be explicit, execution must be evidence-based, and aggregation must preserve the original user goal when tasks fail or are retried.

## What Changes

- Add an AChat-managed `fs_list` tool so agents can inspect workspace directories without brittle shell listing commands.
...
- Record AChat-managed `fs_write` and `bash` outcomes per run and evaluate task reports against declared evidence requirements.
