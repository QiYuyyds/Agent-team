...
AChat MUST handle simple deploy commands without relying on an LLM tool call.

#### Scenario: User sends a simple deploy command

- **WHEN** the user sends `部署`, `发布`, `上线`, or `/deploy`
- **THEN** AChat detects the deployment intent before responder selection
...
- **THEN** AChat deploys that artifact through the same deployment helper used by `deploy_artifact`
...
- **THEN** AChat inserts a system text message explaining that no deployable web app exists.
