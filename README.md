...
Agent-team 是一个基于前端（Next.js + React）和 Python（FastAPI）实现的多 Agent 协作工作空间，把 AI 协作做成 IM 群聊式的体验。

它不把每次 agent 运行当成一段孤立的终端记录，而是围绕「会话」来组织工作：Agent 是联系人，会话是工作空间，文件与产物是共享上下文，Orchestrator 还能把一项工作拆给多个 Agent 并行完成。

<p align="center">
    <img src="docs/images/agenthub-preview.png" alt="Agent-team 多 Agent 协作与产物预览" width="100%" />
</p>
...
- [为什么选 Agent-team](#为什么选-agent-team)
- [功能特性](#功能特性)
...
Agent-team 正是为这套工作流而生。它默认本地运行，使用 SQLite，并把 Agent 的执行保留在你自己的机器上。
...
- 设想的移动端工作方式是「伴随客户端」：手机通过 LAN 或 Tailscale 连到桌面端的 Agent-team host，然后观察运行、发消息、处理审批。
...
后端 Python 依赖通过 `pyproject.toml` 或 `requirements.txt` 管理。
...
移动端被设计成通过 LAN 或 Tailscale 连接桌面端的 Agent-team host。Agent 执行、文件写入、命令执行和 workspace 状态都留在桌面侧。
...
```bash
pnpm dev                 # Web 开发服务
cd backend && python -m uvicorn app.main:app --reload  # Python 后端服务
...
│    AgentRunner、ConversationService  │
│    EventBus、ToolExecutor            │
│  L2 Agent Platform Adapters          │
│    Claude Code、Codex、自定义适配器   │
│  L1 Persistence                      │
