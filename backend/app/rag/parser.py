"""File parsing pipeline — ported from AGI-memory ``internal/document/parser.py``.

Provides ``parse_bytes(filename, content_type, data)`` which extracts plain text
from uploaded files (PDF three-level fallback, or encoding-detected plain text)
and returns a ``ParseResult`` with OCR-need detection.
"""
...
def parse_bytes(filename: str, content_type: str, data: bytes) -> ParseResult:
    ...
    Dispatches to PDF parser for .pdf files, otherwise decodes as text.
    """
...
def _parse_pdf(filename: str, content_type: str, data: bytes) -> ParseResult:
    ...
    for parser_fn in (
        ...
        text, pages, parser_name = parser_fn(data)
        if text.strip():
...
def _normalize_text(text: str) -> str:
    """Fix line endings, remove null bytes, fix hyphenated line breaks, compress blank lines."""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
