"""LLM reranker — listwise re-ranking with fallback to original order.

Ported from AGI-memory ``internal/rag/reranker.py``. Zero modification.
"""
...
class LLMReranker:
    ...
    def rerank(self, query: str, results: List, top_k: int) -> List:
        ...
        try:
            ...
            logger.warning("Rerank failed, fallback to RRF order: %s", e)
            return _truncate(results, top_k)
        if not scores:
            return _truncate(results, top_k)
        ...
        if len(score_map) != len(results):
            logger.warning(
                "Rerank scores count(%d) != candidates(%d), filling missing with 0",
